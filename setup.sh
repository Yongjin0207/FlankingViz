...
setup_rice() {
    ...
    local RAP_REP_URL="https://rapdb.dna.affrc.go.jp/download/archive/irgsp1/IRGSP-1.0_representative_2025-03-19.tar.gz"
    local RAP_PRED_URL="https://rapdb.dna.affrc.go.jp/download/archive/irgsp1/IRGSP-1.0_predicted_2025-03-19.tar.gz"
    ...
    download_if_missing "$RAP_REP_URL" "${DIR}/IRGSP-1.0_representative_2025-03-19.tar.gz"
    test_gzip_or_fail "${DIR}/IRGSP-1.0_representative_2025-03-19.tar.gz"

    download_if_missing "$RAP_PRED_URL" "${DIR}/IRGSP-1.0_predicted_2025-03-19.tar.gz"
    test_gzip_or_fail "${DIR}/IRGSP-1.0_predicted_2025-03-19.tar.gz"

    if [[ ! -d "${DIR}/IRGSP-1.0_representative" ]]; then
        extract_archive_auto "${DIR}/IRGSP-1.0_representative_2025-03-19.tar.gz" "$DIR"
    else
        echo "  - Already extracted: ${DIR}/IRGSP-1.0_representative"
    fi

    if [[ ! -d "${DIR}/IRGSP-1.0_predicted" ]]; then
        extract_archive_auto "${DIR}/IRGSP-1.0_predicted_2025-03-19.tar.gz" "$DIR"
    else
...
setup_soybean() {
    ...
    local SOY_NCBI_BASE="https://ftp.ncbi.nlm.nih.gov/genomes/all/GCF/000/004/515/GCF_000004515.6_Glycine_max_v4.0"
    ...
    download_if_missing \
        "${SOY_NCBI_BASE}/GCF_000004515.6_Glycine_max_v4.0_genomic.fna.gz" \
        "${DIR}/GCF_000004515.6_Glycine_max_v4.0_genomic.fna.gz"
    test_gzip_or_fail "${DIR}/GCF_000004515.6_Glycine_max_v4.0_genomic.fna.gz"
    gunzip_to_if_missing \
        "${DIR}/GCF_000004515.6_Glycine_max_v4.0_genomic.fna.gz" \
        "${DIR}/GCF_000004515.6_Glycine_max_v4.0_genomic.fna"

    download_if_missing \
        "${SOY_NCBI_BASE}/GCF_000004515.6_Glycine_max_v4.0_genomic.gff.gz" \
        "${DIR}/genomic.gff.gz"
    test_gzip_or_fail "${DIR}/genomic.gff.gz"
    gunzip_to_if_missing \
        "${DIR}/genomic.gff.gz" \
        "${DIR}/genomic.gff"
    ...
    write_note "${DIR}/download_sources.txt" \
        "genome_url=${SOY_NCBI_BASE}/GCF_000004515.6_Glycine_max_v4.0_genomic.fna.gz" \
        "ncbi_gff_url=${SOY_NCBI_BASE}/GCF_000004515.6_Glycine_max_v4.0_genomic.gff.gz" \
